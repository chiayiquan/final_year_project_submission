...
import Merchant from "../../libs/Merchant";
import db from "../../db";
...
export default async function UploadMerchant(
  ...
  return new Promise((resolve, reject) => {
    upload(request, response, async (err: Error | string) => {
      ...
      const { countryCode, merchantData, personalInformation } = decodeParams(
      ...
      await db.transaction(async (txn) => {
        ...
        const merchantId = await Merchant.lib.insertMerchant(
          { name: merchantData.name, applicationId },
          merchantData.addresses,
          txn
        );
...
function decodeParams(data: any): Params {
  return JD.object({
    ...
    merchantData: JD.object({
      name: JD.string,
      addresses: JD.array(JD.string),
    }),
